 ...
 */
/**
 * `scope` narrows every Application-based figure to a subset — used to
 * give a recruiter their own jobs' numbers instead of the whole company's.
 * Left as {} (the default), nothing changes for owner/admin: every
 * existing call site keeps working exactly as it did.
 *
 * Revenue is intentionally NOT re-scoped here even when `scope` is set:
 * Payment documents aren't linked to a job directly, and money figures
 * are already hidden from anyone without `viewPayments` regardless (a
 * recruiter doesn't have it) — scoping a number nobody is shown would
 * only add a query for no visible benefit.
 */
export async function getOverview(scope = {}, jobIds = null) {
  await connectDB();
  ...
  const live = { deletedAt: null, ...scope };
  ...
  const [
    ...
    Job.countDocuments({ status: 'published', deletedAt: null, ...(jobIds ? { _id: { $in: jobIds } } : {}) }),

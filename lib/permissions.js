...
export const CAPS = {
  ...
  viewer: {
    ...
    viewEmail: false,
    /**
     * A separate "may look, may not touch" set of flags — Jobs, Payments,
     * and Calling Team were previously gated by the SAME capability that
     * grants the ability to create/delete/verify/assign. That meant
     * giving a Viewer the ability to see any of those pages accidentally
     * also had to give them the ability to act on them. These exist so
     * "can see" and "can change" are decided independently; every other
     * role already sees these pages via their real action capability
     * above, so only Viewer needs them explicitly set to true.
     */
    viewJobsPage: true,
    viewPaymentsPage: true,
    viewCallingTeamPage: true,
    viewJobPerformancePage: true
  }
};
